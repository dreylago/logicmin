from logicmin.binary import countones, binary, numeric, str_to_binary
...
class Cube:
    ...
    def covers(self, minterm):
        mask = self.t | self.f
        return (minterm & mask == self.t) and ((~minterm) & mask == self.f)

    def cover_set(self, minterms):
        return filter(self.covers, minterms)

    def input_pin_cnt(self):
        return countones(self.t, self.MAX_VARS) + countones(
            self.f, self.MAX_VARS
        )
